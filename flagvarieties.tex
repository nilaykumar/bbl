...
V \times B$. \sidenote{See Proposition \ref{principalBbundle}} It follows that:
\begin{align*}
...
\begin{lemma}
\label{charext}
Every character $\sigma : T \to \C^{\times}$ extends uniquely to $B$. \sidenote{reference Man Shun John Ma}
\end{lemma}
\begin{proof}
...
it restricts to 1 on $N$. Consider the differential $d\tilde \sigma|_{\fr n} : \fr n \to \fr{gl}_1(\C)$. Using \sidenote{
reference Borel or Humphreys}, it suffices to prove $d\tilde \sigma|_{\fr n} = 0$, as this implies $\tilde \sigma|_N = 1$.
...
of Serre's twisted sheaves are regular functions on $\A^2$ which are homogenous of degree $k$ \sidenote{Regular functions 
on $\A^2$ and $\A^2 - \{0\}$ are the same, due to the algebraic version of Hartogs' theorem.}:
...
in \sidenote{reference Jantzen}, let $e_1 = [1, 0]^T$ be an element of the standard basis of $\A^2$. Under the standard
...
&= \mathcal{O}(-k)(G/B)
\end{align*}
\sidenote{Figure out how to localize this}
\end{example}
